...
const lookupUser = tool({
  ...
  execute: async ({ name }) => {
    const users: Record<string, object> = {
      alice: { id: "u_101", email: "alice@acme.com", plan: "pro", signupDate: "2024-01-15" },
      bob: { id: "u_102", email: "bob@acme.com", plan: "free", signupDate: "2024-03-22" },
      charlie: { id: "u_103", email: "charlie@acme.com", plan: "enterprise", signupDate: "2023-11-01" },
    };
...
const lookupOrder = tool({
  ...
  execute: async ({ userId }) => {
    const orders: Record<string, object[]> = {
      u_101: [
        { orderId: "ord_501", product: "widget-pro", qty: 2, total: 59.98, status: "shipped" },
        { orderId: "ord_490", product: "gadget-mini", qty: 1, total: 24.99, status: "delivered" },
      ],
      u_102: [
        { orderId: "ord_488", product: "widget-basic", qty: 1, total: 19.99, status: "delivered" },
      ],
      u_103: [
        { orderId: "ord_510", product: "enterprise-suite", qty: 1, total: 999.00, status: "processing" },
      ],
    };
...
const lookupProduct = tool({
  ...
  execute: async ({ slug }) => {
    const products: Record<string, object> = {
      "widget-pro": { name: "Widget Pro", price: 29.99, category: "widgets", inStock: true },
      "widget-basic": { name: "Widget Basic", price: 19.99, category: "widgets", inStock: true },
      "gadget-mini": { name: "Gadget Mini", price: 24.99, category: "gadgets", inStock: false },
      "enterprise-suite": { name: "Enterprise Suite", price: 999.00, category: "enterprise", inStock: true },
    };
  ...
  return await Sentry.withIsolationScope(async () => {
    ...
    try {
      ...
      const result = await generateText({
        model: openai(model),
        system: "You are a helpful assistant with access to a user database, order history, and product catalog. Use the available tools to answer questions.",
        prompt,
      ...
      const tokenEfficiency = promptTokens > 0 ? completionTokens / promptTokens : 0;
      ...
      // Business metrics
      Sentry.metrics.distribution("ai.request.duration", durationMs, { unit: "millisecond", attributes });
      Sentry.metrics.distribution("ai.tokens.prompt", promptTokens, { attributes });
      Sentry.metrics.distribution("ai.tokens.completion", completionTokens, { attributes });
      Sentry.metrics.distribution("ai.tokens.total", totalTokens, { attributes });
      Sentry.metrics.count("ai.request.count", 1, { attributes: { ...attributes, finish_reason: finishReason } });
      Sentry.metrics.distribution("ai.cost", cost, { unit: "dollar", attributes });
      Sentry.metrics.gauge("ai.tokens.efficiency", tokenEfficiency, { attributes });
      Sentry.metrics.distribution("ai.request.steps", stepCount, { attributes });

      // Single structured log with all attributes
      ...
      return new Response(
        JSON.stringify({
          text: result.text,
          steps: stepCount,
          toolCalls: allToolCalls.map((tc) => ({ tool: tc.toolName, args: tc.args })),
        }),
      ...
      Sentry.metrics.count("ai.request.error", 1, {
        attributes: { model, user_id: user.id, error_type: error instanceof Error ? error.name : "Unknown" },
      });
